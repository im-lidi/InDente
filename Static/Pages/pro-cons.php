...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $diagnostico = $_POST["IdDiagnostico"] ?? "";
  ...
  if ($id == "") {
      ...
      (Cedula,IdDoctor,IdDiagnostico,FechaConsulta,Motivo,Observaciones)
      VALUES (?,?,?,?,?,?)");
...
if (isset($_GET["delete"])) {
  ...
  $stmt = $conn->prepare("DELETE FROM Consultas WHERE IdConsulta=?");
...
if (isset($_GET["edit"])) {
  ...
  $stmt = $conn->prepare("SELECT * FROM Consultas WHERE IdConsulta=?");
...
$stmt = $conn->query("SELECT * FROM Consultas");
...
<div class="main">
 ...
 <!-- HEADER -->
  <div class="header">
    ...
    <!-- MENU INTERNO -->
    <div class="tabs">
      <a href="pro-cons.php" class="active">Consultas</a>
  ...
  <div class="content">
    ...
    <div class="dashboard-top">
      <div>
        ...
        <p><?= count($consultas) ?> consultas registrados</p>
      </div>
    ...
    <div class="modal-overlay <?= $editData ? 'show' : '' ?>" id="modal-form">
      <div class="modal">
        <div class="modal-header">
          <h3>Añadir Consulta</h3>
          <button class="modal-close" onclick="cerrarModal('modal-form')">&times;</button>
        </div>

        <form method="POST">
          <input type="hidden" name="IdConsulta" value="<?= $editData['IdConsulta'] ?? '' ?>">
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <input type="number" name="IdDiagnostico" value="<?= $editData['IdDiagnostico'] ?? '' ?>">
            </div>

            <div class="form-group">
              <label>Fecha Consulta</label>
              <input type="date" name="FechaConsulta" value="<?= $editData['FechaConsulta'] ?? '' ?>">
            </div>
          </div>
          ...
          <div class="form-row">
            <div class="form-group">
              <label>Motivo</label>
              <input type="text" name="Motivo" value="<?= $editData['Motivo'] ?? '' ?>">
            </div>

            <div class="form-group">
              <label>Observaciones</label>
              <input type="text" name="Observaciones" value="<?= $editData['Observaciones'] ?? '' ?>">
            </div>
          </div>
...
<?php foreach($consultas as $fila): ?>
      <tr>
        ...
        <td>
          <a href="?edit=<?= $fila['IdConsulta'] ?>" class="btn btn-small btn-green" >Edit</a>
          <a href="?delete=<?= $fila['IdConsulta'] ?>" class="btn btn-small btn-red" onclick="return confirm('¿Deseas eliminar el registro correspondiente al siguiente id = <?= $fila['IdConsulta'] ?> ?')"> Delete </a>
        </td>
      </tr>
<?php endforeach; ?>
    </tbody>
